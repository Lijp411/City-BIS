...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        # Dataset folder
        self.path = '/content/drive/MyDrive/City_BIS/Yuehai' 
        ...
        self.validation_split = 5 #[0, 1, 2, 3, 4, 5]#4
    ...
    def __getitem__(self, batch_i):
        ...
        # print(self.use_potentials) # True
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            if n < 2: #2
                failed_attempts += 1
                if failed_attempts > 100 * self.config.batch_num:  #100
        ...
        elif self.config.in_features_dim == 7: #5
            ...
            for i, room_folder in enumerate(room_folders):
                ...
                for object_name in listdir(join(room_folder, 'Annotations')):
                    ...
                    if object_name[-4:] == '.txt':
                        ...
                        cloud_colors = np.vstack((cloud_colors, object_data[:, 3:6].astype(np.float32)))
    ...
    def load_subsampled_clouds(self):
        ...
        self.sa1_m = PointNetSetAbstraction(64*64, 1.6, 32, 4 + 3, [6,6,12], False) # channels
        self.sa2_m = PointNetSetAbstraction(32*32, 3.2, 32, 12 + 3 , [12,12,16], False) # channels
  
        # Parameter
        dl = self.config.first_subsampling_dl

        # Create path for files
        # loading input_ply subsample???
        ...
        feat_path_1 = join(self.path, 'feat_1_{:.3f}'.format(dl))
        if not exists(feat_path_1):
            makedirs(feat_path_1)

        feat_path_2 = join(self.path, 'feat_2_{:.3f}'.format(dl))
        if not exists(feat_path_2):
            makedirs(feat_path_2)
        
        feat_path_3 = join(self.path, 'feat_3_{:.3f}'.format(dl))
        if not exists(feat_path_3):
            makedirs(feat_path_3)
        ...
        for i, file_path in enumerate(self.files):
            ...
            feat_1_ply_file = join(feat_path_1, '{:s}.ply'.format(cloud_name))
            feat_2_ply_file = join(feat_path_2, '{:s}.ply'.format(cloud_name))
            feat_3_ply_file = join(feat_path_3, '{:s}.ply'.format(cloud_name))
            feat_1_txt_file = join(feat_path_1, '{:s}.txt'.format(cloud_name))
            feat_2_txt_file = join(feat_path_2, '{:s}.txt'.format(cloud_name))
            feat_3_txt_file = join(feat_path_3, '{:s}.txt'.format(cloud_name))
            feat_1_mat_file = join(feat_path_1, '{:s}.mat'.format(cloud_name))
            feat_2_mat_file = join(feat_path_2, '{:s}.mat'.format(cloud_name))
            feat_3_mat_file = join(feat_path_3, '{:s}.mat'.format(cloud_name))
            ...
            if exists(KDTree_file):
                ...
                # Rescale float color and squeeze labe-----new
