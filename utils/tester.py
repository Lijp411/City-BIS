...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        if test_loader.dataset.set == 'validation':
            val_proportions = np.zeros(nc_model, dtype=np.float32)  #posibilty
        ...
        deform_idx=1 #层数
        test_feats = [np.zeros((l.shape[0], 3)) for l in test_loader.dataset.input_labels]
        ...
        while True:
            ...
            for i, batch in enumerate(test_loader):
                ...
                i0 = 0
                #print(s_points) # XYZ batch批次中的点
                #print(stacked_probs) # 分类值 softmax
                #print(in_inds) # 对应点索引
                #print(r) 
                ...
                for b_i, length in enumerate(lengths): #分批 1217(s_points_num)/3(lengths_num)
                    ...
                    #print(in_colors[0])
                    #rint(in_colors[0].shape)
                    #print(batch.features[i0:i0 + length, 1:].cpu().detach().numpy())
                    #print(batch.features[i0:i0 + length, 1:].cpu().detach().numpy().shape)
                    #print(r)

                    #print(points.shape)
                    #print(inds)
                    #op=np.concatenate((points, probs), axis=1)
                    #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/FT.txt", op)
                    #print(r) 
                    ...
                    if 0 < test_radius_ratio < 1:
                        ...
                        probs = probs[mask]
                    '''
                        #in_colors[c_i] = in_colors[c_i][mask]
                        #print(mask.shape)
                        #print(mask)
                    ...
                    #print(self.test_probs[c_i][inds].shape)
                    #print(test_feats[c_i][inds].shape)
                    #print(in_colors[c_i].shape)
                    #print(inds.shape)
                    #print(in_colors[c_i].shape)
                    #print(inds.shape[0]) #inds.shape!=incolors[0].shape ????/
                    #print(in_colors[c_i].shape[0])
                    #print(r)
                    if inds.shape[0]==in_colors[c_i].shape[0]:
                      test_feats[c_i][inds] = test_feats[c_i][inds] + in_colors[c_i]

                    i0 += length
                    #print(self.test_probs[c_i].shape)  
                    #print(test_feats[c_i].shape)  
                
                #print(self.test_probs[0])
                #print(r)
                # 此处获取了一批次的点特征(待softmax值？)
                ...
                if (t[-1] - last_display) > 1.0:
                    ...
                    print(message.format(test_epoch, i,
                                         ...
                                         1000 * (mean_dt[2])))
            #print(self.test_probs[0].shape)
            #print(self.test_probs[1].shape)
            #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/TFF_all.txt", self.test_probs[0])
            #print(r)
            
            #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/New.txt", self.test_probs[0])
            #print(r)
            #print("遍历完一个batch")
            ...
            if last_min + 1 < new_min:
                ...
                if test_loader.dataset.set == 'validation':
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        probs = np.array(self.test_probs[i], copy=True)# 只有一个测区 i的值只有0 只有一批次的值 419
                        ...
                        # Predicted labels
                        preds = test_loader.dataset.label_values[np.argmax(probs, axis=1)].astype(np.int32) #此时预测只有419个值
                        #print(preds.shape)
                        #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/preds.txt", preds)
                        #print(r)

                        # Targets
                        targets = test_loader.dataset.input_labels[i] #target有8w值

                        '''
                        print(targets.shape)
                        np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/preds.txt", targets)
                        print(r)
                        '''
                    ...
                    C *= np.expand_dims(val_proportions / (np.sum(C, axis=1) + 1e-6), 1) # TP/TN/FN/FP
                ...
                # probs实际上等同于了self.test_probs[i][test_loader.dataset.test_proj[i], :] # 在先前的处理过程中更新 i=0

                # 这里处理后的probs是完备的
                if int(np.ceil(new_min)) % 10 == 0:

                    # Project predictions
                    print('\nReproject Vote #{:d}'.format(int(np.floor(new_min)))) #min potential
                    t1 = time.time()
                    proj_probs = []
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        feats = test_feats[i][test_loader.dataset.test_proj[i], :]
                        print(test_loader.dataset.test_proj[i].shape)
                        print(self.test_probs[i].shape)
                        print(feats.shape)
                        

                        np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/probs.txt", probs)
                        np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/tp.txt", test_loader.dataset.test_proj[i])
                        np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/stp.txt", self.test_probs[i])
                        np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/stf.txt", feats)
                        print("1")
                        print(r)
                        
                        #print(self.test_probs[i][test_loader.dataset.test_proj[i], :].shape) #(82647, 2)
                        #print(test_loader.dataset.test_proj[i].shape) (82647,) #投影序号 ？
                        #print(test_loader.dataset.test_proj[i]) #[270 270 270 ...   0   0   0]
                        #print(r)
                        ...
                        #print(probs.shape)
                        #print(probs)
                        #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/New_P.txt", probs)
                        #print(r)
                    ...
                    # Save predictions
                    print('Saving clouds')
                    #np.savetxt("/content/gdrive/MyDrive/KPConv_3/KPConv-PyTorch-master/feature_points/TFF_all3.txt", self.test_probs[0])
